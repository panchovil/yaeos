module yaeos__models_ar_genericcubic
   ...
   abstract interface
      ...
      subroutine abs_Dmix(self, n, T, &
         ...
         real(pr), optional, intent(out)::  dDi(:), dDidT(:), dDij(:, :)
      ...
      subroutine abs_Bmix(self, n, bi, B, dBi, dBij)
         ...
         class(CubicMixRule), intent(in) :: self
         real(pr), intent(in) :: n(:)
         real(pr), optional, intent(in) :: bi(:)
         real(pr), optional, intent(out) :: B, dBi(:), dBij(:, :)
...
contains
   ...
   subroutine GenericCubic_Ar(&
      ...
      dn2 = present(Arn2)
      dn = present(Arn) .or. present(ArVn) .or. present(ArTn) .and. .not. present(Arn2)
      ...
      elseif (dn) then
         call self%mixrule%D1mix(n, self%del1, D1, dD1i)
         call self%mixrule%Bmix(n, self%b, Bmix, dBi)
         call self%mixrule%Dmix(&
            n, T, a, dadt, dadt2, D, dDdT, dDdT2, dDi, dDidT &
            )
      else
         call self%mixrule%D1mix(n, self%del1, D1)
         call self%mixrule%Bmix(n, self%b, Bmix)
         call self%mixrule%Dmix(n, T, a, dadt, dadt2, D, dDdT, dDdT2)
      end if
