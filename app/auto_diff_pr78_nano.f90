...
contains
   ...
   type(PR78_nano_autodiff) function setup(LJ_par, rp, tc, pc, w, kij, lij) result(self)
      ...
      self%LJ_par = LJ_par
      self%rp = rp
      self%alpha_ads_i = ((0.7597_pr*((rp/LJ_par)**-0.7708_pr))-1._pr)/((0.9793_pr*((rp/LJ_par)**(-0.6366_pr)))-1._pr)
      ! self%ac = 0.45723553_pr * R**2 * (self%components%tc**2 / self%components%pc) * &
      ! (1._pr-(0.7597_pr*((rp/LJ_par)**-0.7708_pr)))**2/(1._pr-(0.9793_pr*((rp/LJ_par)**(-0.6366_pr))))
      self%b = 0.07779607_pr * R * (self%components%tc*(1_pr-(0.7597_pr*((rp/LJ_par)**-0.7708_pr))))/&
      ((self%components%pc*(1_pr-(0.9793_pr*((rp/LJ_par)**(-0.6366_pr))))) * self%alpha_ads_i)
      self%ac = 0.45723553_pr * R**2 * ((self%components%tc*(1_pr-(0.7597_pr*((rp/LJ_par)**-0.7708_pr))))**2&
       / (self%components%pc*(1_pr-(0.9793_pr*((rp/LJ_par)**(-0.6366_pr))))))  
   ...
   function arfun(self, n, v, t) result(ar)
      ...
      associate(&
         ...
         ar = (- sum(n) * T * (R*(((1.0_pr/alpha_ads_mix)-1.0_pr)*log(V/sum(n))+&
